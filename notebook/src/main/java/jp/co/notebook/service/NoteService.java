package jp.co.notebook.service;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import jp.co.notebook.entity.Note;
import jp.co.notebook.entity.User;
import jp.co.notebook.repository.NoteRepository;
import jp.co.notebook.repository.UserRepository;

@Service
public class NoteService {
	@Autowired
	NoteRepository noteRepository;
	@Autowired
	UserRepository userRepository;

	public List<Note> handleFindByUserOrderByIdMax5(User user) {
		List<Note> list = this.noteRepository.findByUserOrderByNoteDateDesc(user);
		while (list.size() > 5) {
			list.remove(list.size() - 1);
		}
		return list;
	}

	public boolean checkAuthorization(String email, long id) {
		User authUser = this.userRepository.findByEmail(email);
		User noteUser = this.noteRepository.findById(id).getUser();
		if (authUser.equals(noteUser)) {
			return true;
		} else {
			return false;
		}
	}

	public List<Note> handleFindAllByUser(User user) {
		return this.noteRepository.findByUserOrderByNoteDateDesc(user);
	}

	public long handleUpdateNote(long id, Note note) {
		this.noteRepository.deleteById(id);
		this.noteRepository.save(note);
		return this.noteRepository.findByNoteDate(note.getNoteDate()).getId();
	}

	public void handleSaveNote(Note note) {
		this.noteRepository.save(note);
	}

	public Note handleFindById(long id) {
		return this.noteRepository.findById(id);
	}

	public void handleRemoveById(long id) {
		this.noteRepository.deleteById(id);
	}
}
